...
import { focusDayText, focusHourCoverText, focusHourNowText, focusHourWindowButton, focusStreakText, hourSpan, limitSoonText, suggestButton, windowButton } from './popup-core.js';
...
function paintHits(state) {
  ...
  hitsNote.textContent = `Ma ez a ${n}. megakadás`
    ...
    + peakNowText(peakNow(state, today, new Date().getHours()))
    + peakDayNowText(peakDayNow(state, today, new Date().getDay())) + hitsNudge(n);
...
async function paintStart() {
  try {
    ...
    + limitSoonText(link, Date.now(), CLOSED_FRESH_MS);
