...
final class DigestTests: XCTestCase {
    ...
    func testTheUsageWeekdayInTheSentenceIsTheStatsLineAndNeedsMeasurement() {
        var day = full
        day.usageWeekday = (day: 6, count: 12000)
        XCTAssertEqual(DigestLogic.text(day) { $0 },
                       "Elmúlt 7 nap: 7 ó 20 p mért idő; a legtöbb: youtube.com 2 ó 40 p (▼ -33% az előző héthez képest). "
                       + "A négy hét legnagyobb napja: szombat (átlag 50 p). 9 menet (7 ó 0 p, 2 korán leállítva). 3 feloldás.")
        var none = full
        none.usageWeekday = nil
        XCTAssertEqual(DigestLogic.text(none) { $0 }, DigestLogic.text(full) { $0 }, "nap nélkül a régi mondat")
        var unmeasured = day
        unmeasured.last7Seconds = 0
        var plain = full
